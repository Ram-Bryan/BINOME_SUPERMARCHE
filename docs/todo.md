...
- [ ] Créer la base de données SQLite + les 5 tables (voir `schema.sql`) — 30mn
- [ ] Exécuter les inserts (5 produits, 2 caisses, 1 utilisateur test) — inclus dans `schema.sql`
- [ ] Initialiser le projet CodeIgniter — 5mn
- [ ] Se mettre d'accord sur : noms des clés de session (`user_id`, `caisse_id`, `panier`), arborescence des vues, config de la connexion SQLite dans `app/Config/Database.php`

## Étudiant A — Authentification, navigation & session (~95mn)

- [ ] Créer le template à partir du fichier fourni — 25mn
